...
def plot(w_cPDS, b_cPDS, w_SSVM, b_SSVM, xtrain, xtest, ytrain, ytest, classes):
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=xtrain[ytrain == classes[0], 0], y=xtrain[ytrain == classes[0], 1], mode='markers', name='training class 0'))
    fig.add_trace(
        go.Scatter(x=xtrain[ytrain == classes[1], 0], y=xtrain[ytrain == classes[1], 1], mode='markers', name='training class 1'))
    fig.add_trace(go.Scatter(x=xtest[ytest == classes[0], 0], y=xtest[ytest == classes[0], 1], mode='markers', name='test class 0'))
    fig.add_trace(go.Scatter(x=xtest[ytest == classes[1], 0], y=xtest[ytest == classes[1], 1], mode='markers', name='test class 1'))
